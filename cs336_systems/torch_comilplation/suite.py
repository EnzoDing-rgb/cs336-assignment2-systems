"""torch.compile benchmark suite: attention operator (a) + end-to-end (b)."""
...
from cs336_systems.e2e_timing.e2e import (
    ARTIFACTS_ROOT as E2E_ARTIFACTS_ROOT,
    PART_B_WARMUP,
    SUITE_SEED,
    SUITE_STEPS,
    BenchmarkConfig,
...
# --- attention (a) ---
ATTN_ARTIFACTS = REPO_ROOT / "artifacts" / "attention_operator"
ATTN_EAGER_PATH = ATTN_ARTIFACTS / "results.json"
ATTN_COMPILE_PATH = ATTN_ARTIFACTS / "compile_results.json"
REPORT_PATH = REPO_ROOT / "reports" / "benchmarking-torch-compile.md"

ATTN_PALETTE = {16: "#4c72b0", 32: "#55a868", 64: "#c44e52", 128: "#8172b3"}
EAGER_STYLE = {"linestyle": "-", "linewidth": 2.2, "alpha": 0.95}
COMPILED_STYLE = {"linestyle": "--", "linewidth": 2.2, "alpha": 0.95}

# --- e2e (b) ---
E2E_COMPILE_ARTIFACTS = E2E_ARTIFACTS_ROOT / "compile_suite"
E2E_MANIFEST_PATH = E2E_COMPILE_ARTIFACTS / "manifest.json"
E2E_COMPILE_EXTRA_WARMUP = 5
E2E_SIZES = ("small", "medium", "large", "xl")


def _img(figures_dir: Path, name: str, width: int = 640) -> str:
    return f'<img src="{figures_dir / name}" alt="{name}" width="{width}" />'
...
def _attn_key(r: dict[str, Any]) -> tuple[int, int]:
    return (r["d_model"], r["seq_len"])


def _attn_lookup(results: list[dict[str, Any]]) -> dict[tuple[int, int], dict[str, Any]]:
    return {_attn_key(r): r for r in results}


def run_attention(*, skip_run: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if skip_run:
        eager_raw = json.loads(ATTN_EAGER_PATH.read_text(encoding="utf-8"))["results"]
        compiled_raw = json.loads(ATTN_COMPILE_PATH.read_text(encoding="utf-8"))["results"]
        return eager_raw, compiled_raw

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required")
    ATTN_ARTIFACTS.mkdir(parents=True, exist_ok=True)
    print("── [a] compiled attention sweep ──", flush=True)
    compiled_results = run_sweep(device=torch.device("cuda"), use_compile=True, extend_for_oom=False)
    ATTN_COMPILE_PATH.write_text(
        json.dumps(
            {
                "results": [r.to_dict() for r in compiled_results],
                "use_compile": True,
    ...
    print(f"Wrote {ATTN_COMPILE_PATH}", flush=True)
    eager_raw = json.loads(ATTN_EAGER_PATH.read_text(encoding="utf-8"))["results"]
    return eager_raw, [r.to_dict() for r in compiled_results]


def _attn_plot_curves(
    eager_map: dict,
    compiled_map: dict,
    field: str,
    filename: str,
    ...
    d_vals = sorted({k[0] for k in eager_map})
    fig, ax = plt.subplots(figsize=(9, 5.2))
    for d in d_vals:
        keys = sorted([k for k in eager_map if k[0] == d and not eager_map[k].get("oom")], key=lambda x: x[1])
        xs = [k[1] for k in keys]
        ye = [eager_map[k][field] for k in keys]
        yc = [compiled_map[k][field] for k in keys if not compiled_map[k].get("oom")]
        xs_c = [k[1] for k in keys if not compiled_map[k].get("oom")]
        color = ATTN_PALETTE.get(d, "#333333")
        ax.plot(xs, ye, "o", color=color, label=f"d={d} eager", markeredgecolor="white", **EAGER_STYLE)
        if xs_c:
            ax.plot(xs_c, yc, "s", color=color, label=f"d={d} compiled", markeredgecolor="white", **COMPILED_STYLE)
    ax.set_xscale("log", base=2)
    ax.set_xlabel("序列长度 S")
    ax.set_ylabel("均值时间 (ms)")
    ax.set_title(f"scaled dot-product attention · {title_cn} · eager vs torch.compile · B=8", fontsize=11, pad=12)
    ax.legend(ncol=2, fontsize=8, frameon=True, edgecolor="#dddddd")
...
def _attn_plot_speedup_heatmap(
    eager_map: dict, compiled_map: dict, field: str, filename: str, title_cn: str
) -> Path:
    out = FIGURES_DIR / filename
    d_vals = sorted({k[0] for k in eager_map})
    s_vals = sorted({k[1] for k in eager_map})
    data = np.full((len(d_vals), len(s_vals)), np.nan)
    for i, d in enumerate(d_vals):
        for j, s in enumerate(s_vals):
            k = (d, s)
            e, c = eager_map.get(k), compiled_map.get(k)
            if e and c and not e.get("oom") and not c.get("oom") and e[field] and c[field]:
                data[i, j] = e[field] / c[field]
    fig, ax = plt.subplots(figsize=(10, 4.2))
    cmap = plt.cm.RdYlGn.copy()
    cmap.set_bad(color="#eeeeee")
    im = ax.imshow(np.ma.masked_invalid(data), aspect="auto", cmap=cmap, vmin=0.8, vmax=2.5)
    for i, d in enumerate(d_vals):
        for j, s in enumerate(s_vals):
            val = data[i, j]
            if np.isfinite(val):
                ax.text(j, i, f"{val:.2f}×", ha="center", va="center", fontsize=8)
            elif eager_map.get((d, s), {}).get("oom") or compiled_map.get((d, s), {}).get("oom"):
                ax.text(j, i, "OOM", ha="center", va="center", fontsize=8, color="#8b0000", fontweight="bold")
    ...
    ax.set_title(f"{title_cn}（eager ÷ compiled，>1 表示编译版更快）", fontsize=11, pad=12)
...
def _attn_plot_grouped_bars(eager_map: dict, compiled_map: dict) -> Path:
    out = FIGURES_DIR / "tc_attn_grouped_speedup_4096_16384.png"
    highlight_s = [4096, 16384]
    d_vals = sorted({k[0] for k in eager_map})
    x = np.arange(len(d_vals))
    width = 0.32
    fig, axes = plt.subplots(1, 2, figsize=(11, 4.8), sharey=True)

    for ax, s, title in zip(axes, highlight_s, ["S = 4096", "S = 16384"]):
        fwd_speedup = []
        bwd_speedup = []
        for d in d_vals:
            e, c = eager_map[(d, s)], compiled_map[(d, s)]
            fwd_speedup.append(e["forward_ms"] / c["forward_ms"])
            bwd_speedup.append(e["backward_ms"] / c["backward_ms"])
        bars_fwd = ax.bar(x - width / 2, fwd_speedup, width, label="前向加速比", color="#55a868", edgecolor="#333")
        bars_bwd = ax.bar(x + width / 2, bwd_speedup, width, label="反向加速比", color="#8172b3", edgecolor="#333")
        ax.axhline(1.0, color="#888888", linestyle="--", linewidth=1.2, label="eager 基线 (=1)")
        for bars in (bars_fwd, bars_bwd):
            for bar in bars:
                h = bar.get_height()
                ax.text(bar.get_x() + bar.get_width() / 2, h + 0.03, f"{h:.2f}×", ha="center", va="bottom", fontsize=8)
        ax.set_xticks(x)
        ax.set_xticklabels([f"d={d}" for d in d_vals])
        ax.set_ylabel("加速比（eager 归一化为 1，>1 表示 compiled 更快）")
        ax.set_ylim(0.9, max(max(fwd_speedup), max(bwd_speedup)) * 1.15)
        ax.set_title(title, fontsize=11)
        ax.grid(axis="y", alpha=0.35)

    axes[0].legend(fontsize=8, loc="upper left")
    fig.suptitle("代表性序列长度：前向 / 反向加速比（eager = 1）", fontsize=12, y=1.02)
    fig.tight_layout()
    fig.savefig(out, dpi=160, bbox_inches="tight")
    plt.close(fig)
    return out
...
def make_attention_figures(eager: list[dict[str, Any]], compiled: list[dict[str, Any]]) -> dict[str, Path]:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    eager_map = _attn_lookup(eager)
    compiled_map = _attn_lookup(compiled)
    return {
        "forward": _attn_plot_curves(
            eager_map, compiled_map, "forward_ms", "tc_attn_forward_eager_vs_compiled.png", "前向时间"
        ),
        "backward": _attn_plot_curves(
            eager_map, compiled_map, "backward_ms", "tc_attn_backward_eager_vs_compiled.png", "反向时间"
        ),
        "speedup_fwd": _attn_plot_speedup_heatmap(
            eager_map, compiled_map, "forward_ms", "tc_attn_forward_speedup_heatmap.png", "前向加速比"
        ),
        "speedup_bwd": _attn_plot_speedup_heatmap(
            eager_map, compiled_map, "backward_ms", "tc_attn_backward_speedup_heatmap.png", "反向加速比"
        ),
        "bars": _attn_plot_grouped_bars(eager_map, compiled_map),
    }


def _load_attention_data() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    eager = json.loads(ATTN_EAGER_PATH.read_text(encoding="utf-8"))["results"]
    compiled = json.loads(ATTN_COMPILE_PATH.read_text(encoding="utf-8"))["results"]
    return eager, compiled


def _load_e2e_data() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    manifest = json.loads(E2E_MANIFEST_PATH.read_text(encoding="utf-8"))
    return manifest["vanilla"], manifest["compiled"]


def _render_attention_section(eager: list[dict[str, Any]], compiled: list[dict[str, Any]]) -> list[str]:
    eager_map = {(r["d_model"], r["seq_len"]): r for r in eager}
    compiled_map = {(r["d_model"], r["seq_len"]): r for r in compiled}
    lines = [
        ...
        "",
        f"**原始数据：** `{ATTN_EAGER_PATH}` · `{ATTN_COMPILE_PATH}`",
        "",
        "### 1. 前向时间对比",
        "",
        _img(FIGURES_DIR, "tc_attn_forward_eager_vs_compiled.png"),
        "",
        ...
        "### 2. 反向时间对比",
        "",
        _img(FIGURES_DIR, "tc_attn_backward_eager_vs_compiled.png"),
        "",
        ...
        "### 3. 加速比热力图",
        "",
        _img(FIGURES_DIR, "tc_attn_forward_speedup_heatmap.png", 700),
        "",
        _img(FIGURES_DIR, "tc_attn_backward_speedup_heatmap.png", 700),
        "",
        ...
        "### 4. 代表性序列长度加速比柱图",
        "",
        _img(FIGURES_DIR, "tc_attn_grouped_speedup_4096_16384.png", 700),
        "",
    ...
    for s in sorted({k[1] for k in eager_map if k[0] == 16}):
        e, c = eager_map.get((16, s)), compiled_map.get((16, s))
        if not e:
            continue
        if e.get("oom") or (c and c.get("oom")):
            lines.append(f"| {s} | — | — | — | — | — | OOM |")
            continue
        if not c:
            continue
        fs = e["forward_ms"] / c["forward_ms"]
        bs = e["backward_ms"] / c["backward_ms"]
        lines.append(
            f"| {s} | {e['forward_ms']:.2f} | {c['forward_ms']:.2f} | {fs:.2f}× | "
            f"{e['backward_ms']:.2f} | {c['backward_ms']:.2f} | {bs:.2f}× |"
        )
    lines.extend(
        [
            "",
...
def run_attention_suite(*, skip_run: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    eager, compiled = run_attention(skip_run=skip_run)
    figs = make_attention_figures(eager, compiled)
...
def _e2e_ms(seg: dict | None, name: str) -> float | None:
    if not seg or seg.get("oom"):
        return None
    stats = seg.get("segments", {}).get(name)
    if not stats:
        return None
    return stats["mean_s"] * 1000.0


def _e2e_full_step_ms(r: dict) -> float | None:
    if r.get("oom"):
        return None
    return r.get("step_mean_s", 0) * 1000.0


def run_e2e(*, skip_run: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if skip_run:
        manifest = json.loads(E2E_MANIFEST_PATH.read_text(encoding="utf-8"))
        return manifest["vanilla"], manifest["compiled"]

    E2E_COMPILE_ARTIFACTS.mkdir(parents=True, exist_ok=True)
    vanilla: list[dict[str, Any]] = []
    compiled: list[dict[str, Any]] = []

    for size in E2E_SIZES:
        print(f"── [b] vanilla {size} ──", flush=True)
        vcfg = BenchmarkConfig(
            model_size=size,
            mode="timed_train",
            warmup=PART_B_WARMUP,
            steps=SUITE_STEPS,
            seed=SUITE_SEED,
            use_compile=False,
        )
        vanilla.append(_run_cell(vcfg, reuse=True))

        print(f"── [b] compiled {size} ──", flush=True)
        ccfg = BenchmarkConfig(
            model_size=size,
            mode="timed_train",
            warmup=PART_B_WARMUP + E2E_COMPILE_EXTRA_WARMUP,
            steps=SUITE_STEPS,
            seed=SUITE_SEED,
            use_compile=True,
        )
        try:
            compiled.append(run_benchmark(ccfg, artifacts_root=E2E_COMPILE_ARTIFACTS).to_dict())
        except torch.cuda.OutOfMemoryError as err:
            compiled.append({"config": ccfg.__dict__, "oom": True, "error": str(err)})

    E2E_MANIFEST_PATH.write_text(
        json.dumps(
            {"vanilla": vanilla, "compiled": compiled, "generated_utc": datetime.now(timezone.utc).isoformat()},
            indent=2,
        ),
        encoding="utf-8",
    )
...
E2E_SEGMENTS = ("forward", "backward", "optimizer")
E2E_SEGMENT_LABELS = {"forward": "forward", "backward": "backward", "optimizer": "optimizer"}
E2E_SEGMENT_COLORS = {"forward": "#4c72b0", "backward": "#c44e52", "optimizer": "#8172b3"}


def _e2e_collect_stats(
    vanilla: list[dict], compiled: list[dict]
) -> list[dict[str, float | str]]:
    rows: list[dict[str, float | str]] = []
    for size in E2E_SIZES:
        v = next((r for r in vanilla if r.get("config", {}).get("model_size") == size), None)
        c = next((r for r in compiled if r.get("config", {}).get("model_size") == size), None)
        if not v or not c or v.get("oom") or c.get("oom"):
            continue
        seg_v = {s: _e2e_ms(v, s) for s in E2E_SEGMENTS}
        seg_c = {s: _e2e_ms(c, s) for s in E2E_SEGMENTS}
        if any(x is None for x in seg_v.values()) or any(x is None for x in seg_c.values()):
            continue
        total_v = sum(seg_v[s] for s in E2E_SEGMENTS)  # type: ignore[operator]
        rows.append(
            {
                "size": size,
                **{f"v_{s}": seg_v[s] for s in E2E_SEGMENTS},
                **{f"c_{s}": seg_c[s] for s in E2E_SEGMENTS},
                **{f"sp_{s}": seg_v[s] / seg_c[s] for s in E2E_SEGMENTS},  # type: ignore[operator]
                **{f"pct_{s}": 100.0 * seg_v[s] / total_v for s in E2E_SEGMENTS},  # type: ignore[operator]
                "total_v": total_v,
                "total_c": sum(seg_c[s] for s in E2E_SEGMENTS),  # type: ignore[operator]
            }
        )
    return rows


def make_e2e_figures(vanilla: list[dict], compiled: list[dict]) -> dict[str, Path]:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    paths: dict[str, Path] = {}
    stats = _e2e_collect_stats(vanilla, compiled)
    if not stats:
        return paths

    sizes = [str(r["size"]) for r in stats]
    x = np.arange(len(sizes))
    n_seg = len(E2E_SEGMENTS)
    w = 0.22

    # Figure 1: speedup (left) + time share (right)
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.8))
    ax_sp, ax_pct = axes

    for i, seg in enumerate(E2E_SEGMENTS):
        speedups = [r[f"sp_{seg}"] for r in stats]
        offset = (i - 1) * w
        bars = ax_sp.bar(x + offset, speedups, w, label=seg, color=E2E_SEGMENT_COLORS[seg], edgecolor="#333")
    ...
    ax_sp.axhline(1.0, color="#888888", linestyle="--", linewidth=1.2)
    ax_sp.set_xticks(x)
    ax_sp.set_xticklabels(sizes)
    ax_sp.set_ylabel("加速比（vanilla ÷ compiled）")
    ax_sp.set_title("三段加速比随模型规模变化", fontsize=11)
    ax_sp.legend(fontsize=8)
    ax_sp.grid(axis="y", alpha=0.35)
    ax_sp.set_ylim(0.9, max(r[f"sp_{s}"] for r in stats for s in E2E_SEGMENTS) * 1.18)

    bottom = np.zeros(len(sizes))
    for seg in E2E_SEGMENTS:
        pcts = [r[f"pct_{seg}"] for r in stats]
        ax_pct.bar(x, pcts, 0.55, bottom=bottom, label=seg, color=E2E_SEGMENT_COLORS[seg], edgecolor="#333")
        for xi, pct in enumerate(pcts):
            if pct >= 5:
                ax_pct.text(xi, bottom[xi] + pct / 2, f"{pct:.0f}%", ha="center", va="center", fontsize=8, color="white")
        bottom += pcts

    ax_pct.set_xticks(x)
    ax_pct.set_xticklabels(sizes)
    ax_pct.set_ylabel("占三步合计时间（%）")
    ax_pct.set_title("vanilla 三步时间占比（不含 loss）", fontsize=11)
    ax_pct.legend(fontsize=8, loc="upper right")
    ax_pct.set_ylim(0, 100)

    fig.suptitle("端到端训练步：加速比 × 时间占比", fontsize=12, y=1.02)
    fig.tight_layout()
    p1 = FIGURES_DIR / "tc_e2e_speedup_and_share.png"
    fig.savefig(p1, dpi=160, bbox_inches="tight")
    plt.close(fig)
    paths["speedup_share"] = p1

    # Figure 2: vanilla vs compiled ms — three segments side by side
    fig, axes = plt.subplots(1, 3, figsize=(12, 4.2), sharey=False)
    bw = 0.32
    for ax, seg in zip(axes, E2E_SEGMENTS):
        vvals = [r[f"v_{seg}"] for r in stats]
        cvals = [r[f"c_{seg}"] for r in stats]
        ax.bar(x - bw / 2, vvals, bw, label="vanilla", color="#4c72b0", edgecolor="#333")
        ax.bar(x + bw / 2, cvals, bw, label="compiled", color="#55a868", edgecolor="#333")
        ax.set_xticks(x)
        ax.set_xticklabels(sizes)
        ax.set_title(f"{seg}", fontsize=11)
        ax.set_ylabel("时间 (ms)")
        ax.grid(axis="y", alpha=0.35)
        for i, (vv, cv) in enumerate(zip(vvals, cvals)):
            sp = vv / cv
            ymax = max(vvals[i], cvals[i])
            ax.text(i, ymax * 1.02, f"{sp:.2f}×", ha="center", va="bottom", fontsize=8, color="#333333")
    axes[0].legend(fontsize=8)
    fig.suptitle("三段绝对时间：vanilla vs torch.compile（柱顶标注加速比）", fontsize=12, y=1.02)
    fig.tight_layout()
    p2 = FIGURES_DIR / "tc_e2e_segment_ms_compare.png"
...
def _render_e2e_section(vanilla: list[dict], compiled: list[dict]) -> list[str]:
    stats = _e2e_collect_stats(vanilla, compiled)
    lines = [
    ...
    for r in stats:
        lines.append(
            f"| {r['size']} | {r['sp_forward']:.2f}× | {r['sp_backward']:.2f}× | {r['sp_optimizer']:.2f}× | "
            f"{r['pct_forward']:.1f}% | {r['pct_backward']:.1f}% | {r['pct_optimizer']:.1f}% |"
        )
    lines.extend(
        [
            "",
            _img(FIGURES_DIR, "tc_e2e_speedup_and_share.png", 720),
            "",
            "左图：三步加速比随模型规模的变化。",
            "右图：三步时间占比——backward 六成、forward 三成、optimizer 一成，四档不变。",
            "",
            _img(FIGURES_DIR, "tc_e2e_segment_ms_compare.png", 720),
            "",
    ...
    for r in stats:
        lines.append(
            f"| {r['size']} | {r['c_forward']:.0f} | {r['v_forward'] - r['c_forward']:.1f} | "
            f"{r['c_backward']:.0f} | {r['v_backward'] - r['c_backward']:.1f} |"
        )
    lines.extend(
        [
            "",
...
def render_report(
    ...
    attn_compiled: list[dict[str, Any]],
    vanilla: list[dict[str, Any]],
    e2e_compiled: list[dict[str, Any]],
) -> str:
    lines = [
    ...
    lines.extend(_render_attention_section(eager, attn_compiled))
    lines.extend(["", "---", ""])
    lines.extend(_render_e2e_section(vanilla, e2e_compiled))
    return "\n".join(lines)


def run_e2e_suite(*, skip_run: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    vanilla, compiled = run_e2e(skip_run=skip_run)
    figs = make_e2e_figures(vanilla, compiled)
    print("Figures:", ", ".join(str(p) for p in figs.values()), flush=True)
    return vanilla, compiled


def write_report(
    attn: tuple[list[dict[str, Any]], list[dict[str, Any]]] | None = None,
    e2e: tuple[list[dict[str, Any]], list[dict[str, Any]]] | None = None,
) -> None:
    if attn is None:
        attn = _load_attention_data()
    if e2e is None:
        e2e = _load_e2e_data()
    REPORT_PATH.write_text(render_report(*attn, *e2e), encoding="utf-8")
...
def main() -> None:
    p = argparse.ArgumentParser(description="torch.compile benchmarks (attention + e2e)")
    p.add_argument(
        "--part",
        choices=("all", "attention", "e2e"),
        default="all",
        help="which benchmark to run (default: all)",
    )
    p.add_argument("--skip-run", action="store_true", help="rebuild figures/reports from saved JSON only")
    args = p.parse_args()

    attn_data: tuple[list[dict[str, Any]], list[dict[str, Any]]] | None = None
    e2e_data: tuple[list[dict[str, Any]], list[dict[str, Any]]] | None = None

    if args.part in ("all", "attention"):
        attn_data = run_attention_suite(skip_run=args.skip_run)
    if args.part in ("all", "e2e"):
        e2e_data = run_e2e_suite(skip_run=args.skip_run)

    write_report(attn=attn_data, e2e=e2e_data)
