...
> 对应 handout 的 `tp_calcs`。设定与 [data-parallel-calculations.md](./data-parallel-calculations.md) 相同：单设备算力 $C$（FLOP/s）、出向带宽 $W$（字节/s）、激活与梯度均为 FP16（每元素 2 字节）。这里不再切 batch，而是把 **权重矩阵的某一维** 切到 $N_{\mathrm{TP}}$ 台设备上。
...
| 符号 | 含义 |
|------|------|
| $B$ | batch（样本数；本课设定里 TP **不切** batch，每台都看完整 $B$） |
...
| $x$ | 输入，形状 $(B, D)$；**每台都有完整的一份** |
| $x_1^{(i)}, x_2^{(i)}, z^{(i)}$ | 设备 $i$ 上的中间激活，形状 $\bigl(B,\tfrac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}\bigr)$ |
| $y^{(i)}$ | 设备 $i$ 的局部输出，形状 $(B, D)$ |
| $y$ | all-reduce 之后的完整输出，形状 $(B, D)$ |
...
目标：$y = xW$，其中 $x$ 形状 $(B,D)$，$W$ 形状 $(D, D_{\mathrm{FF}})$。
...
$$
y^{(i)} = x\,W^{(i)}
\qquad\text{形状}\ \Bigl(B,\ \tfrac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}\Bigr).
$$
...
同时把 $x$ 按列切成 $x^{(i)}$，形状 $\bigl(B,\tfrac{D}{N_{\mathrm{TP}}}\bigr)$，然后

$$
y^{(i)} = x^{(i)}\,W^{(i)}
\qquad\text{形状}\ (B, D_{\mathrm{FF}}).
$$
...
给定完整输入 $x\in\mathbb{R}^{B\times D}$。设备 $i$ 持有：
...
| $y^{(i)}$ | 用自己的行分片把宽维块压回 $(B,D)$，得到部分和 | 否 |
| $y$ | 把各台部分和加总，每台都拿到完整输出 | **是：all-reduce** |

前向里 **唯一** 的集合通信，就是最后这一次对形状 $(B,D)$ 张量的 all-reduce。
...
未分片时，前向是 $x_1=xW_1,\ x_2=xW_2,\ z=f(x_1)\odot x_2,\ y=zW_3$。给定 $dy$（形状 $(B,D)$）：
...
handout 假定每台都拿到同一份 $dy\in\mathbb{R}^{B\times D}$（前向 all-reduce 之后每台都有完整 $y$，后续损失对 $y$ 的梯度在各台上一致）。
...
$dz^{(i)}$ 形状 $\bigl(B,\tfrac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}\bigr)$，正好只覆盖本台负责的那一段宽维。
...
| | 集合通信 | 张量形状 |
|--|----------|----------|
| 前向 | 1 次 all-reduce（汇总 $y^{(i)}$） | $(B,D)$ |
| 反向 | 1 次 all-reduce（汇总 $dx^{(i)}_{\mathrm{local}}$） | $(B,D)$ |
...
$$
\begin{aligned}
x\,W_1^{(i)} &: \quad 2\,B\,D\,\frac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}, \\
x\,W_2^{(i)} &: \quad 2\,B\,D\,\frac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}, \\
z^{(i)}\,W_3^{(i)} &: \quad 2\,B\,\frac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}\,D
= 2\,B\,D\,\frac{D_{\mathrm{FF}}}{N_{\mathrm{TP}}}.
\end{aligned}
$$
...
一句话：未分片前向是 $6BD D_{\mathrm{FF}}$；TP 把每个 matmul 的宽维（或等价的收缩维）缩成 $1/N_{\mathrm{TP}}$，故每台计算量除以 $N_{\mathrm{TP}}$。

### 4.2 反向

仍是六个 matmul（三个权重梯度 + 三个激活侧梯度），每个代价同为 $2\,B\,D\,D_{\mathrm{FF}}/N_{\mathrm{TP}}$：
...
对比数据并行：DP 是用 $B/N_{\mathrm{DP}}$ 除以设备数；TP 是用 $D_{\mathrm{FF}}/N_{\mathrm{TP}}$ 除以设备数。两者都把每台 FLOP 降到原来的 $1/N$，但切的轴不同。

---

## 5. 通信时间怎么数（本题 (c)）

通信对象始终是形状 $(B,D)$ 的 FP16 张量，字节数：
...
一句话（前向）：前向只需对 $(B,D)$ 的输出做一次环形 all-reduce，耗时 $\frac{4(N_{\mathrm{TP}}-1)BD}{N_{\mathrm{TP}} W}$。  
一句话（反向）：反向只需对 $(B,D)$ 的输入梯度做一次同样的 all-reduce，耗时相同。

注意：$D_{\mathrm{FF}}$ **不进**通信量——宽维分片留在各台本地，跨设备传的永远是窄的 $(B,D)$。
...
&=
\frac{6\,B\,D\,D_{\mathrm{FF}}}{N_{\mathrm{TP}}\,C}, \\[0.4em]
...
1. **$B$ 与 $D$ 被约掉了。** 通信量 $\propto BD$，计算量 $\propto BD D_{\mathrm{FF}}$，比值只剩 $D_{\mathrm{FF}}$ 与机器的 $C/W$。
...
- 前向：$\dfrac{6\,B\,D\,D_{\mathrm{FF}}}{N_{\mathrm{TP}}}$  
  （三个分片 matmul，每个 $2BD D_{\mathrm{FF}}/N_{\mathrm{TP}}$。）
- 反向：$\dfrac{12\,B\,D\,D_{\mathrm{FF}}}{N_{\mathrm{TP}}}$  
  （六个分片 matmul，每个同上。）
...
- 前向与反向均为
  ...
  \frac{4(N_{\mathrm{TP}}-1)\,BD}{N_{\mathrm{TP}}\,W}
  $$
  （各一次对 $(B,D)$ FP16 张量的环形 all-reduce。）
...
（令 $T_{\mathrm{comm}}\ge T_{\mathrm{compute}}$，约掉 $B,D$ 后解出。）
